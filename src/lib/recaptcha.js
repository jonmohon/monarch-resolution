// reCAPTCHA Enterprise (score-based) loader + token fetch for the lead form.
// Fail-open by design: any failure (no site key, blocked script, not ready)
// resolves to an empty token so the form still submits — the server's honeypot,
// time-trap, rate limit, and input validation remain in force regardless.
...
export function loadRecaptcha() {
  ...
  loadingPromise = new Promise((resolve) => {
    ...
    s.async = true;
    s.onload = () => resolve(window.grecaptcha?.enterprise || null);
